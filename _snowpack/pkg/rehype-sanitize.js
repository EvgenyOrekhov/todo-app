import { i as immutable } from './common/immutable-935f0f91.js';
...
function wrapper(node, schema) {
  ...
  if (node && typeof node === 'object' && node.type) {
    replace = one(immutable(defaults, schema || {}), node, []);
...
function handleProperties(schema, properties, node, stack) {
  ...
  var allowed = immutable(toPropertyValueMap(schema.attributes['*']), toPropertyValueMap(own.call(schema.attributes, name) ? schema.attributes[name] : []));
  var result = {};
  var definition;
  var key;
  var value;
